...
SELECT
  ...
  '42144335',
  'Hebisawa Y',
  'Reply to the Letter by Koga et al.',
  'Internal medicine (Tokyo, Japan)',
  '2026 May 16',
  'https://doi.org/10.2169/internalmedicine.7570-26',
  0,
  1,
  'Prova 3',
  1,
...
WHERE NOT EXISTS (
  SELECT 1 FROM core_publication WHERE pmid='42144335'
);

UPDATE core_publication
SET
  authors = CASE WHEN authors IS NULL OR authors='' THEN 'Hebisawa Y' ELSE authors END,
  title = CASE WHEN title IS NULL OR title='' THEN 'Reply to the Letter by Koga et al.' ELSE title END,
  journal = CASE WHEN journal IS NULL OR journal='' THEN 'Internal medicine (Tokyo, Japan)' ELSE journal END,
  publication_date = CASE WHEN publication_date IS NULL OR publication_date='' THEN '2026 May 16' ELSE publication_date END,
  url = CASE WHEN url IS NULL OR url='' THEN 'https://doi.org/10.2169/internalmedicine.7570-26' ELSE url END,
  ...
  submission_notes = CASE
    WHEN submission_notes IS NULL OR submission_notes='' THEN 'Prova 3'
...
VALUES
  ('Haemophilus influenzae Rd KW20', 'Haemophilus influenzae Rd KW20', NULL,
   0, NULL, 'Prova 3',
   datetime('now'), (SELECT curator_id FROM core_curator ORDER BY curator_id LIMIT 1), (SELECT publication_id FROM core_publication WHERE pmid='42144335' LIMIT 1), datetime('now'), NULL);

INSERT INTO core_curation_TF_instances (curation_id, tfinstance_id)
SELECT (SELECT curation_id FROM core_curation WHERE publication_id=(SELECT publication_id FROM core_publication WHERE pmid='42144335' LIMIT 1) ORDER BY curation_id DESC LIMIT 1), (SELECT TF_instance_id FROM core_tfinstance WHERE uniprot_accession='P0A126' LIMIT 1)
WHERE NOT EXISTS (
  SELECT 1 FROM core_curation_TF_instances
  WHERE curation_id=(SELECT curation_id FROM core_curation WHERE publication_id=(SELECT publication_id FROM core_publication WHERE pmid='42144335' LIMIT 1) ORDER BY curation_id DESC LIMIT 1) AND tfinstance_id=(SELECT TF_instance_id FROM core_tfinstance WHERE uniprot_accession='P0A126' LIMIT 1)
);
...
SELECT
  (SELECT genome_id FROM core_genome WHERE genome_accession='NC_000907.1' LIMIT 1),
  'WP_005686672.1',
  'ACP-like domain-containing protein',
  274307,
  274753,
  -1,
  'HI_RS01325',
...
WHERE NOT EXISTS (
  ...
  WHERE genome_id=(SELECT genome_id FROM core_genome WHERE genome_accession='NC_000907.1' LIMIT 1)
    AND start=2815 AND end=2821 AND strand=1
    AND _seq='AAAATGT'
);
...
VALUES
  ((SELECT curation_id FROM core_curation WHERE publication_id=(SELECT publication_id FROM core_publication WHERE pmid='42144335' LIMIT 1) ORDER BY curation_id DESC LIMIT 1),
...
SELECT (SELECT id FROM core_curation_siteinstance
        WHERE curation_id=(SELECT curation_id FROM core_curation WHERE publication_id=(SELECT publication_id FROM core_publication WHERE pmid='42144335' LIMIT 1) ORDER BY curation_id DESC LIMIT 1)
...
SELECT
  ...
  (SELECT gene_id FROM core_gene
            WHERE locus_tag='HI_RS00010' AND genome_id=(SELECT genome_id FROM core_genome WHERE genome_accession='NC_000907.1' LIMIT 1)
...
WHERE NOT EXISTS (
  SELECT 1 FROM core_siteinstance
  WHERE genome_id=(SELECT genome_id FROM core_genome WHERE genome_accession='NC_000907.1' LIMIT 1)
    AND start=1530 AND end=1536 AND strand=1
    AND _seq='TGTAAAA'
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM core_siteinstance
  WHERE genome_id=(SELECT genome_id FROM core_genome WHERE genome_accession='NC_000907.1' LIMIT 1)
    AND start=3963 AND end=3969 AND strand=1
    AND _seq='AATGTAA'
);
...
SELECT
  (SELECT id FROM core_curation_siteinstance
        ...
        ORDER BY id DESC LIMIT 1),
  (SELECT gene_id FROM core_gene
            WHERE locus_tag='HI_RS00015' AND genome_id=(SELECT genome_id FROM core_genome WHERE genome_accession='NC_000907.1' LIMIT 1)
            ORDER BY gene_id DESC LIMIT 1),
  'exp_verified',
  NULL
WHERE (SELECT gene_id FROM core_gene
