...
SELECT
  'ARTICLE',
  '41652288',
  'McKown S, Arnold B, Boucher F, Correia H, Eremenco S, Geršić T, Johnson M, Koumbarou A, Matias I, Parks-Vernizzi E, Poepsel T, Son J, Strouse E, Terwee CB, Wade M',
  ...
  '2026 Feb 6',
  'https://doi.org/10.1186/s41687-026-01012-5',
  0,
  1,
  'Test 2',
  1,
...
WHERE NOT EXISTS (
  SELECT 1 FROM core_publication WHERE pmid='41652288'
);

UPDATE core_publication
SET
  authors = CASE WHEN authors IS NULL OR authors='' THEN 'McKown S, Arnold B, Boucher F, Correia H, Eremenco S, Geršić T, Johnson M, Koumbarou A, Matias I, Parks-Vernizzi E, Poepsel T, Son J, Strouse E, Terwee CB, Wade M' ELSE authors END,
  title = CASE WHEN title IS NULL OR title='' THEN 'Use of AI within COA linguistic validation and eCOA migration processes: analysis and good practice recommendations.' ELSE title END,
  journal = CASE WHEN journal IS NULL OR journal='' THEN 'Journal of patient-reported outcomes' ELSE journal END,
  publication_date = CASE WHEN publication_date IS NULL OR publication_date='' THEN '2026 Feb 6' ELSE publication_date END,
  url = CASE WHEN url IS NULL OR url='' THEN 'https://doi.org/10.1186/s41687-026-01012-5' ELSE url END,
  reported_TF = CASE WHEN reported_TF IS NULL OR reported_TF='' THEN 'VsrD' ELSE reported_TF END,
  ...
  submission_notes = CASE
    WHEN submission_notes IS NULL OR submission_notes='' THEN 'Test 2'
    ELSE submission_notes
  END
WHERE pmid='41652288';

INSERT INTO core_tf (name, family_id, description)
SELECT 'VsrD', 2, 'A response regulator [PFAM::PF00072] of the LuxR family in Ralstonia solanacearum involved in regulation of exopolysaccharide synthesis [PMID::9573161].'
WHERE NOT EXISTS (
  SELECT 1 FROM core_tf WHERE lower(name)=lower('VsrD')
);

UPDATE core_tf
SET
  family_id = COALESCE(family_id, 2),
  description = CASE WHEN description IS NULL THEN 'A response regulator [PFAM::PF00072] of the LuxR family in Ralstonia solanacearum involved in regulation of exopolysaccharide synthesis [PMID::9573161].' ELSE description END
WHERE lower(name)=lower('VsrD');

INSERT INTO core_tfinstance (refseq_accession, uniprot_accession, description, TF_id, notes)
SELECT
  'WP_003424711',
  'Q18B74',
  'stage 0 sporulation protein A [Peptoclostridium difficile 630].',
  (SELECT TF_id FROM core_tf WHERE lower(name)=lower('VsrD') LIMIT 1),
  ''
WHERE NOT EXISTS (
  SELECT 1 FROM core_tfinstance WHERE uniprot_accession='Q18B74'
);

UPDATE core_tfinstance
SET
  TF_id = COALESCE(TF_id, (SELECT TF_id FROM core_tf WHERE lower(name)=lower('VsrD') LIMIT 1)),
  refseq_accession = COALESCE(NULLIF(refseq_accession,''), 'WP_003424711'),
  description = COALESCE(NULLIF(description,''), 'stage 0 sporulation protein A [Peptoclostridium difficile 630].'),
  notes = COALESCE(notes, '')
WHERE uniprot_accession='Q18B74';
...
VALUES
  ('Helicobacter pylori 26695', 'Helicobacter pylori 26695', NULL,
   0, NULL, 'Test 2',
   datetime('now'), (SELECT curator_id FROM core_curator ORDER BY curator_id LIMIT 1), (SELECT publication_id FROM core_publication WHERE pmid='41652288' LIMIT 1), datetime('now'), NULL);

INSERT INTO core_curation_TF_instances (curation_id, tfinstance_id)
SELECT (SELECT curation_id FROM core_curation WHERE publication_id=(SELECT publication_id FROM core_publication WHERE pmid='41652288' LIMIT 1) ORDER BY curation_id DESC LIMIT 1), (SELECT TF_instance_id FROM core_tfinstance WHERE uniprot_accession='Q18B74' LIMIT 1)
WHERE NOT EXISTS (
  SELECT 1 FROM core_curation_TF_instances
  WHERE curation_id=(SELECT curation_id FROM core_curation WHERE publication_id=(SELECT publication_id FROM core_publication WHERE pmid='41652288' LIMIT 1) ORDER BY curation_id DESC LIMIT 1) AND tfinstance_id=(SELECT TF_instance_id FROM core_tfinstance WHERE uniprot_accession='Q18B74' LIMIT 1)
);
...
SELECT 'Copper-phenanthroline footprinting', 'Copper-phenanthroline footprinting', NULL, 'ECO:0005626'
WHERE NOT EXISTS (
  SELECT 1 FROM core_experimentaltechnique WHERE EO_term='ECO:0005626'
);
...
WHERE NOT EXISTS (
  ...
  WHERE genome_id=(SELECT genome_id FROM core_genome WHERE genome_accession='NC_000915.1' LIMIT 1)
    AND start=700 AND end=705 AND strand=1
    AND _seq='ATTGCC'
);
...
VALUES
  ((SELECT curation_id FROM core_curation WHERE publication_id=(SELECT publication_id FROM core_publication WHERE pmid='41652288' LIMIT 1) ORDER BY curation_id DESC LIMIT 1),
...
SELECT
  (SELECT id FROM core_curation_siteinstance
        WHERE curation_id=(SELECT curation_id FROM core_curation WHERE publication_id=(SELECT publication_id FROM core_publication WHERE pmid='41652288' LIMIT 1) ORDER BY curation_id DESC LIMIT 1)
...
WHERE NOT EXISTS (
  SELECT 1 FROM core_siteinstance
  WHERE genome_id=(SELECT genome_id FROM core_genome WHERE genome_accession='NC_000915.1' LIMIT 1)
    AND start=2220 AND end=2231 AND strand=-1
    AND _seq='CCTTGGAAACTG'
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM core_siteinstance
  WHERE genome_id=(SELECT genome_id FROM core_genome WHERE genome_accession='NC_000915.1' LIMIT 1)
    AND start=767 AND end=776 AND strand=1
    AND _seq='CTGACCGTCA'
);
